...
function Show({ tab, radixes }: { tab: string, radixes: Radix[] }) {
	...
	return <Tables>
		{ radixes.map(radix => {
			...
			return <Table key={`show-${radix.name}`} tab="show" numbers={numbers} rows={rows} cols={cols} radix={radix} low={lowest} high={highest} mainRow={mainRow}/>
		})}
	</Tables>
}
