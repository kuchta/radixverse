...
export default function Header({ radixes, updateRadixes }: {
	...
	useLayoutEffect(() => {
		...
		let style = { width: `${length}ex`, height: `${Math.round(1.7 * rootREM)}px` }
		if (screenWidth > xl) {
			setFormColumn(false)
		} else if (screenWidth > md) {
			setFormColumn(false)
			if (length > 55) style = { width: `${length / 2}ex`, height: `${Math.round(3.3 * rootREM)}px` }
		} else {
			setFormColumn(true)
			if (length > 55) style = { width: `${length / 3}ex`, height: `${Math.round(4.7 * rootREM)}px` }
		}
		setInputStyle(style)
